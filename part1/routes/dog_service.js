...
router.get('/dogs', async (req, res) => {
    ...
    // Users.username FROM Dogs INNER JOIN Users ON Dogs.owner_id = user_id');

    // Adjusted for Part 2
    const [rows] = await db.query('SELECT Dogs.dog_id, Dogs.name, Dogs.size, Dogs.owner_id FROM Dogs');
    res.json(rows);
});
