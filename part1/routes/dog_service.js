...
router.get('/walkers/summary', async (req, res) => {
    const [rows] = await db.query('SELECT Users.username, COUNT(WalkRatings.rating) AS TotalRatings, AVG(WalkRatings.ratings) AS AvgRating, COUNT(DISTINCT WalkRequests.request_id) AS WalksCompleted FROM Users LEFT JOIN Walk');
    res.json(rows);
});
